...
@export_group("Áudio de Passos")
@export var vel_som_andando: float = 1.6
@export var vel_som_correndo: float = 2.3
@export var volume_dos_passos: float = -12.0 # Reduzido em decibéis
...
var som_passos: AudioStreamPlayer3D
...
var trilho_atual: PathFollow3D = null # Guarda o carrinho que a Rafael está usando no momento
...
var direcao_camera_alvo: float = 0.0
...
# --- REFERÊNCIAS DE NÓS ---
@onready var animation_player: AnimationPlayer = $Rafael/AnimationPlayer
@onready var visual_model: Node3D = $Rafael
...
func _ready():
	...
	if cam:
		...
		_cam_ray.position = Vector3(0, 1.5, 0) # Raio sai do peito da Rafael
		_cam_ray.target_position = _original_cam_pos - _cam_ray.position
		
		# O RayCast só precisa bater no cenário (Layer 1). A Rafael está na Layer 2, então ele ignora ela!
	...
	var tex_vazio = load("res://Fase2/Assets/UI/coração sem vida.png")
	var tex_cheio = load("res://Fase2/Assets/UI/coração com vida.png")
	...
	# --- CRIAR SOM DE PASSOS ---
	som_passos = AudioStreamPlayer3D.new()
	var stream_passos = load("res://Fase2/Personagens/andandoConcreto.mp3")
	if stream_passos and stream_passos is AudioStreamMP3:
		stream_passos.loop = true
	som_passos.stream = stream_passos
	som_passos.volume_db = volume_dos_passos # Volume abaixado e ajustável
	som_passos.position = Vector3(0, -0.8, 0) # Toca na altura dos pés
	add_child(som_passos)
	
	# --- CRIAR LANTERNA NA MÃO DA RAFAEL ---
	var lanterna = SpotLight3D.new()
	lanterna.name = "Lanterna"
	lanterna.spot_range = 15.0 # Alcance da luz
	lanterna.spot_angle = 35.0 # Abertura do cone de luz
	lanterna.light_color = Color(1.0, 0.95, 0.8) # Luz levemente amarelada (quente)
	lanterna.light_energy = 3.0 # Força da luz
	lanterna.shadow_enabled = true # Ativa sombras para ficar mais realista
	
	# Adicionamos a lanterna como filha do modelo visual para ela girar junto com a Rafael
	visual_model.add_child(lanterna)
	
	# Posição aproximada da mão direita dela
	# O eixo Z positivo é a "frente" da Rafael e o X negativo é a "direita" dela
	lanterna.position = Vector3(-0.4, 0.8, 0.5)
	
	# Gira a lanterna para ela apontar para a direção que a Rafael está olhando
	# O SpotLight3D por padrão aponta para -Z. Como a Rafael olha para +Z, giramos 180 graus.
	lanterna.rotation_degrees = Vector3(0, 180, 0)
...
func _process(delta: float) -> void:
	...
	# Controle de emissão das partículas de passos e do som
	if particulas_passos != null:
		var andando = abs(velocity.x) > 0.1 or abs(velocity.z) > 0.1
		var no_chao = is_on_floor() or is_on_path
		var deve_tocar = (andando and no_chao)
		
		particulas_passos.emitting = deve_tocar
		
		if is_instance_valid(som_passos):
			if deve_tocar:
				# Acelera o som dos passos se a Rafael estiver correndo, ou mantém mais normal se estiver andando
				som_passos.pitch_scale = vel_som_correndo if esta_correndo else vel_som_andando
...
func iniciar_curva(novo_angulo_y: float, duracao: float, angulo_camera_y: float = -999.0, centro_alvo: Vector3 = Vector3(INF, INF, INF)):
	...
	# Centraliza a Rafael no eixo perpendicular à NOVA direção para mantê-la no meio do corredor
	if centro_alvo.x != INF:
		...
		# Se a nova direção for no eixo Z (90 graus ou 270 graus), o eixo lateral é o X
	...
	var current_input = Input.get_axis("move_left", "move_right")
	if current_input < 0:
		angulo_camera_y += PI
	...
	if current_input < 0:
		angulo_visual_alvo = novo_angulo_y - PI/2
...
func handle_movement(delta: float) -> void:
	...
	# Calcula a direção da câmera baseada no input
	if input_axis > 0:
		direcao_camera_alvo = angulo_movimento
	elif input_axis < 0:
		direcao_camera_alvo = angulo_movimento + PI
		
	# Suaviza a rotação da câmera (se não estiver no meio de uma curva automática)
	if _camera_pivot and not fazendo_curva:
		var diff_cam = fposmod(direcao_camera_alvo - _camera_pivot.rotation.y + PI, TAU) - PI
		_camera_pivot.rotation.y = lerp_angle(_camera_pivot.rotation.y, _camera_pivot.rotation.y + diff_cam, 10 * delta)
	
	# O personagem sempre se move na direção para onde a câmera está apontando
	var dir_x = cos(direcao_camera_alvo) * abs(input_axis)
	var dir_z = -sin(direcao_camera_alvo) * abs(input_axis)
	...
	if input_axis != 0:
		...
		# Rotação visual do modelo: sempre de costas para a câmera (correndo para frente)
		if not fazendo_curva:
			# Como a malha original da Rafael tem offset de -90 graus, para ela ficar de costas 
			# para a câmera (correndo "para o fundo"), adicionamos PI/2 ao ângulo da câmera.
			var rotacao_alvo = direcao_camera_alvo + PI/2
			var diff_vis = fposmod(rotacao_alvo - visual_model.global_rotation.y + PI, TAU) - PI
			visual_model.global_rotation.y = lerp_angle(visual_model.global_rotation.y, visual_model.global_rotation.y + diff_vis, 10 * delta)
	else:
...
func receber_dano(quantidade: float) -> void:
	vida -= quantidade
	print("Rafael recebeu dano do morcego! Vida atual: ", int(vida))
	if vida <= 0:
		print("Rafael morreu!")
