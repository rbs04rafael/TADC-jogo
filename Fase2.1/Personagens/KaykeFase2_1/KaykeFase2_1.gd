...
var trilho_atual: PathFollow3D = null # Guarda o carrinho que a Kayke está usando no momento
...
# --- VARIÁVEIS DE CHECKPOINT (PARKOUR) ---
...
func _ready():
	...
	# Busca a lanterna na mão da Kayke (adicionada pelo nosso script)
	...
	if cam:
		...
		# O RayCast só precisa bater no cenário (Layer 1). A Kayke está na Layer 2, então ele ignora ela!
	...
	if has_node("/root/Global"):
		...
		if global.porta_destino_nome != "":
			...
			if porta:
				# Teleporta a Kayke para a posição da porta + um deslocamento para a frente dela
				# Usa -Z da base global da porta normalizada para garantir que o 'Scale' da porta não empurre a Kayke mais longe do que o esperado
...
func socar() -> void:
	...
	print("Kayke desferiu um soco rápido!")
	var hitbox = find_child("HitboxSoco", true, false)
	if hitbox:
...
func equipar_lanterna() -> void:
	...
	# Faz o próprio corpo da Kayke brilhar com as cores originais no escuro
	ativar_brilho_corpo()
...
func iniciar_curva(novo_angulo_y: float, duracao: float, angulo_camera_y: float = -999.0, centro_alvo: Vector3 = Vector3(INF, INF, INF), cam_offset: Vector3 = Vector3.ZERO):
	...
	# Centraliza a Kayke no eixo perpendicular à NOVA direção para evitar que ela caia da beirada
	...
	camera_parkour_alvo_y = angulo_camera_y # Salva o novo alvo da câmera do parkour
	...
	# Mudei de + PI/2 para - PI/2 para inverter a direção que a Kayke fica virada!
...
func _physics_process(delta: float) -> void:
	...
	if entrou_no_parkour and global_position.y < altura_limite:
		...
		if checkpoint_ativo:
			...
			# Se ainda não chegou no Bloco 1, volta para o início do Parkour
	...
	# Faz a lanterna sempre apontar para frente da Kayke, independente da animação da mão
	if ref_lanterna and tem_lanterna:
		var mao = ref_lanterna.get_parent()
		if mao:
			# Direção para frente baseada no modelo visual (Kayke)
			...
			if frente.length() > 0.1:
				...
				# Força a luz (SpotLight3D) a apontar para a frente da Kayke!
				if ref_spotlight:
...
func entrar_na_escada(escada: Area3D) -> void:
	...
	print("Kayke começou a escalar a escada!")
...
func handle_movement(delta: float) -> void:
	...
	# INVERSÃO DINÂMICA DO A/D (D sempre vai para a direita da tela, A para a esquerda)
	if get_tree().current_scene.name == "Fase2.1" and transicao_camera_pronta:
	...
	if direction:
		...
		# Rotação visual do modelo baseada na direção de input
		var rotacao_alvo = angulo_movimento + PI/2
		if input_axis == -1:
			rotacao_alvo = angulo_movimento - PI/2
			
		# FASE 2 ABSOLUTE ANTI-MOONWALK: Força o modelo a olhar para onde a velocidade física está indo
		if get_tree().current_scene.name == "Fase2.1":
			if velocity.length_squared() > 0.01:
				# atan2(x, z) é a rotação global exata para um modelo importado virado para +Z
...
func handle_path_movement(delta: float) -> void:
	...
	if direction != 0:
		...
		# Se chegar no fim do caminho (0.99+), FORCE a saída do caminho
			...
			# Subtraímos PI (180 graus) porque o modelo 3D da Kayke foi importado virado de costas pro padrão do Godot (+Z)
			var rotacao_alvo = rotacao_caminho - PI
			
			# Compensação legada extra apenas para a Fase 1 (tinha um erro a mais de 57 graus no Parque)
			if get_tree().current_scene.name != "Fase2.1":
			...
			# Usamos global_rotation.y para ignorar a rotação do nó pai (Kayke)
			visual_model.global_rotation.y = lerp_angle(visual_model.global_rotation.y, rotacao_alvo, 8 * delta)
	
	# Atualiza a posição X e Z para prender no caminho, mas deixa o Y livre para pulos
	if is_on_path and trilho_atual != null:
		...
		# --- SISTEMA FÍSICO HÍBRIDO (X/Z pelo Path, Y pela Física Real) ---
		if not is_on_floor():
			velocity.y -= gravity * delta
		else:
			pulos_dados = 0
			
		# Comando de pular usando a física real
		if Input.is_action_just_pressed("move_up") and pulos_dados < max_pulos:
			velocity.y = jump_velocity
			pulos_dados += 1
			if animation_player:
		...
		# Aplica a física para que ela caia se não houver bloco embaixo dela!
		move_and_slide()
		
		# Restaura a velocidade fictícia para que o sistema de animações saiba que ela está andando
		velocity.x = direction * current_speed
		
		# Mantém a câmera sempre atrás da Kayke enquanto ela estiver no caminho
		var deve_virar_camera = false
		if trilho_atual.get_parent() and "Parque" in trilho_atual.get_parent().name:
			deve_virar_camera = true
		elif get_tree().current_scene.name == "Fase2.1":
			deve_virar_camera = true
			
		if _camera_pivot and deve_virar_camera:
			# A câmera segue a tangente do trilho, IGNORANDO se a Kayke virou para trás
			var alvo_camera = trilho_atual.global_rotation.y
			
			# Compensação legada apenas para a Fase 1
			if get_tree().current_scene.name != "Fase2.1":
...
func checar_zona_parkour(delta: float) -> void:
	if get_tree().current_scene.name == "Fase2.1" and not is_on_path:
		# Se a Kayke estiver perto do Rubber Block e ainda não entrou
		if global_position.x > 120.0 and global_position.z < 25.0 and not entrou_no_parkour:
			...
			if not travar_eixo_z:
				...
				global_position.z = z_inicial # Força o encaixe imediato no trilho 2D
				angulo_movimento = 0 # Move no eixo X
			
			# Delay de 0.6 segundos antes da câmera puxar pro lado
			get_tree().create_timer(0.6).timeout.connect(func(): 
				transicao_camera_pronta = true
				# Força a Kayke a olhar para a DIREITA da tela (-90 graus) quando o 2D começa
				visual_model.rotation.y = deg_to_rad(-90.0)
			)
				
		# Atualiza a câmera lateral continuamente (desde que não esteja na animação de virar a esquina)
		if entrou_no_parkour and not fazendo_curva and transicao_camera_pronta:
			if _camera_pivot:
				# Reduzido de 4.0 para 2.0 para fazer o giro da câmera ser mais suave
				...
				#  câmera parkour X(profundidade) Y(altura) Z(lado)
				_target_cam_pos = Vector3(-10.0, 2.5, 1.0)

		# GATILHOS MANUAIS DE CURVA (Resolve o problema do Area3D que não detectava a Kayke)
		if entrou_no_parkour and not fazendo_curva:
			# Chegou no Bloco_Virada1 (X cruzou 117.5)
			# Usamos angulo_movimento_y = 90.0 para que a Kayke corra em direção ao Z negativo ao pressionar D
			...
			# (Espaço reservado para os próximos blocos...)
...
func entrar_no_caminho(carrinho: PathFollow3D) -> void:
	...
	# FORÇA A KAYKE A MOVER IMEDIATAMENTE PARA O CARRINHO
...
func _on_virada_parque_entered(body: Node3D, index: int, area: Area3D) -> void:
	...
	print("Kayke entrou na Virada ", index, ". Apontando exatamente para a próxima área!")
	
	# Busca o centro exato da colisão (CollisionShape3D) para alinhar a Kayke,
