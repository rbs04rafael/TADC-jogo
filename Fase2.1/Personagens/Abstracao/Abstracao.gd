...
func _ready() -> void:
	# Encontra a Kayke na cena, já que agora as abstrações são colocadas manualmente
	kayke = get_tree().get_root().find_child("Kayke", true, false)
...
func _check_flashlight() -> void:
	...
	if "lanterna_ligada" in kayke and "tem_lanterna" in kayke:
		if kayke.lanterna_ligada and kayke.tem_lanterna:
			var spot = kayke.ref_spotlight
...
func _become_furious() -> void:
	is_furious = true
	print("Abstração ficou FURIOSA com a luz da lanterna e está perseguindo a Kayke!")
	if health_bar_sprite:
		health_bar_sprite.visible = true

func _chase_kayke(delta: float) -> void:
	var to_kayke = kayke.global_position - global_position
	to_kayke.y = 0 # Ignora eixo Y para mover só no chão
	if to_kayke.length() > 0.5:
		var dir = to_kayke.normalized()
		# Corre atrás da Kayke
		velocity.x = move_toward(velocity.x, dir.x * speed, 15.0 * delta)
		velocity.z = move_toward(velocity.z, dir.z * speed, 15.0 * delta)
		
		# Faz a abstração olhar para a Kayke, mas inverte a direção ( - dir ) 
...
func _check_damage(delta: float) -> void:
	...
	if attack_cooldown <= 0.0:
		for i in get_slide_collision_count():
			...
			if collider == kayke:
				if kayke.has_method("receber_dano"):
					kayke.receber_dano(damage, global_position)
					attack_cooldown = 1.0 # 1 segundo de cooldown para o próximo ataque
					
					# Empurrão leve pra trás após bater
					var knockback = (global_position - kayke.global_position).normalized()
